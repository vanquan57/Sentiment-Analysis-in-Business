# =============================================================================
# train_model.py — Benchmark 3 models trên Multi-Domain / Multi-Language
# Môi trường: Google Colab (GPU T4 hoặc A100)
# Models: mBERT | XLM-RoBERTa | PhoBERT
# =============================================================================
...
# Cách A: upload thẳng lên Colab session
# DATA_DIR = Path("benchmarks_datasets/multi_domain")

# Cách B: Google Drive
from google.colab import drive
drive.mount('/content/drive')
DATA_DIR = Path("/content/drive/MyDrive/Chuyên Đề 4/Multi-domain/datasets")

TRAIN_PATH = DATA_DIR / "train.csv"
VAL_PATH   = DATA_DIR / "val.csv"
TEST_PATH  = DATA_DIR / "test.csv"

OUT_DIR = Path("results/multi_domain")
...
if "domain" in train_df.columns:
    print(f"\nDistribution by domain (train):")
    print(train_df.groupby(["domain", "label"]).size().unstack().fillna(0).astype(int))
...
class SentimentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # token_type_ids: mBERT có, XLM-RoBERTa không có — xử lý an toàn
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler):
    ...
    for batch in tqdm(loader, desc="  Training", leave=False):
        ...
        # chỉ truyền token_type_ids nếu batch thực sự có giá trị khác 0
        extra = {}
...
# Muốn chạy từng cái: result = train_model("mBERT", MODEL_CONFIGS["mBERT"])
# =============================================================================
...
fig.suptitle("Benchmark: Multi-Domain EN — mBERT vs XLM-RoBERTa", fontsize=13, fontweight="bold")
...
ax.set_title("Validation F1 Macro per Epoch — Multi-Domain EN")
ax.set_xlabel("Epoch")
...
fig.suptitle("Normalized Confusion Matrix — Multi-Domain EN", fontsize=13, fontweight="bold")
plt.tight_layout()
...
# =============================================================================
# STEP 9 — Phân tích F1 theo từng domain (amazon / twitter / yelp)
# =============================================================================

def analyze_per_domain(results: dict, df: pd.DataFrame):
    if "domain" not in df.columns:
        print("Cột 'domain' không tồn tại, bỏ qua per-domain analysis.")
        return
    ...
    for model_key, res in results.items():
        ...
        for domain in sorted(tmp["domain"].unique()):
            ...
            rows.append({"Model": model_key, "domain": domain, "F1 Macro": round(f1, 4)})

    per_domain_df = pd.DataFrame(rows)
    pivot = per_domain_df.pivot(index="domain", columns="Model", values="F1 Macro")
    print("\nPer-domain F1 Macro:")
    print(pivot.to_string())
    per_domain_df.to_csv(OUT_DIR / "per_domain_f1.csv", index=False)

    fig, ax = plt.subplots(figsize=(7, 4))
    sns.heatmap(pivot, annot=True, fmt=".4f", cmap="YlOrRd",
                ax=ax, linewidths=0.5, vmin=0.5, vmax=1.0)
    ax.set_title("F1 Macro per Domain — Multi-Domain EN")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "heatmap_per_domain.png", dpi=150, bbox_inches="tight")
    plt.show()
    print("Saved: heatmap_per_domain.png")


analyze_per_domain(all_results, test_df)
...
# =============================================================================
# STEP 11 — Domain Adaptation: Unseen Domain Transfer
# Train trên N-1 domains, test trên 1 domain hoàn toàn unseen lúc train
# Đây mới là domain adaptation đúng nghĩa
# =============================================================================

def train_on_subset(model_key: str, cfg: dict, train_subset_df: pd.DataFrame,
                    val_subset_df: pd.DataFrame, save_path: Path) -> None:
    """Train model trên subset data, save best theo val_f1."""
    tokenizer = AutoTokenizer.from_pretrained(cfg["model_name"], use_fast=True)
    model = AutoModelForSequenceClassification.from_pretrained(
        cfg["model_name"],
        num_labels=NUM_LABELS,
    ...
    train_ds  = SentimentDataset(train_subset_df, tokenizer, cfg["max_length"])
    val_ds    = SentimentDataset(val_subset_df,   tokenizer, cfg["max_length"])
    ...
    print(f"    ✓ Saved (best_val_f1={best_val_f1:.4f}) → {save_path}")


def domain_adaptation_unseen(model_configs: dict,
                              full_train_df: pd.DataFrame,
                              full_val_df:   pd.DataFrame,
                              full_test_df:  pd.DataFrame):
    """
    Leave-one-domain-out TRAINING experiment.

    Với mỗi held-out domain:
      - Train trên (all domains - held_out)
      - Val   trên (all domains - held_out)
      - Test  trên held_out domain ONLY  ← unseen hoàn toàn lúc train

    So sánh với baseline (train trên all domains).
    """
    if "domain" not in full_test_df.columns:
        print("Không có cột 'domain', bỏ qua.")
        return

    domains = sorted(full_test_df["domain"].unique())
    da2_dir = OUT_DIR / "domain_adaptation_unseen"
    da2_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"  DOMAIN ADAPTATION — Unseen Domain Transfer")
    print(f"  Held-out domains: {domains}")
    print(f"{'='*60}")
    ...
    for held_out in domains:
        ...
        # ── Tách train/val: bỏ held_out domain ──
        train_sub = full_train_df[full_train_df["domain"] != held_out].reset_index(drop=True)
        val_sub   = full_val_df  [full_val_df  ["domain"] != held_out].reset_index(drop=True)

        # ── Test: chỉ held_out domain ──
        test_sub  = full_test_df [full_test_df ["domain"] == held_out].reset_index(drop=True)

        print(f"    train={len(train_sub):,} | val={len(val_sub):,} | test_unseen={len(test_sub):,}")

        for model_key, cfg in model_configs.items():
            print(f"\n    Model: {model_key}")
            save_path = da2_dir / f"{model_key}_{held_out}"

            # ── Train trên domains trừ held_out ──
            train_on_subset(model_key, cfg, train_sub, val_sub, save_path)

            # ── Evaluate trên unseen held_out ──
            ...
            test_ds    = SentimentDataset(test_sub, tokenizer, cfg["max_length"])
            test_loader = DataLoader(test_ds, batch_size=cfg["batch_size"],
                                     shuffle=False, num_workers=0, pin_memory=False)
            metrics = evaluate(best_model, test_loader, desc="Test-unseen")

            # ── Lấy baseline score (all-domain model) từ da_results ──
            baseline_f1 = None
            if model_key in da_results and held_out in da_results[model_key]:
                baseline_f1 = da_results[model_key][held_out]["f1_macro"]

            delta = round(metrics["f1_macro"] - baseline_f1, 4) if baseline_f1 else None

            print(f"    unseen_f1={metrics['f1_macro']:.4f} | "
                  f"baseline_f1={baseline_f1} | delta={delta}")

            rows.append({
                "Model":          model_key,
                "Held-out Domain": held_out,
                "Train Domains":  str([d for d in domains if d != held_out]),
                "Unseen F1":      round(metrics["f1_macro"],  4),
                "Unseen Acc":     round(metrics["accuracy"],  4),
    ...
    # ── Bảng tổng hợp ──
    da2_df = pd.DataFrame(rows)
    print(f"\n{'='*60}")
    print("  DOMAIN ADAPTATION — Unseen Domain Results")
    print('='*60)
    print(da2_df.to_string(index=False))
    da2_df.to_csv(da2_dir / "results.csv", index=False)

    # ── Plot: Unseen F1 vs Baseline F1 ──
    fig, axes = plt.subplots(1, len(domains), figsize=(5 * len(domains), 5))
    if len(domains) == 1:
        axes = [axes]

    for ax, held_out in zip(axes, domains):
        sub = da2_df[da2_df["Held-out Domain"] == held_out]
        x        = np.arange(len(sub))
        width    = 0.35
        models   = sub["Model"].tolist()
        baseline = sub["Baseline F1"].tolist()
        unseen   = sub["Unseen F1"].tolist()

        bars1 = ax.bar(x - width/2, baseline, width, label="Baseline (all domains)",
                       color="#AAAAAA", alpha=0.85)
        bars2 = ax.bar(x + width/2, unseen,   width, label="Unseen transfer",
                       color=[COLORS.get(m, "#888") for m in models], alpha=0.85)
    ...
    fig.suptitle("Domain Adaptation — Unseen vs Baseline F1", fontsize=13, fontweight="bold")
    plt.tight_layout()
    plt.savefig(da2_dir / "unseen_vs_baseline.png", dpi=150, bbox_inches="tight")
    plt.show()
    print("Saved: unseen_vs_baseline.png")

    # ── Heatmap Delta F1 ──
    pivot_delta = da2_df.pivot(index="Held-out Domain", columns="Model", values="Delta F1")
    fig, ax = plt.subplots(figsize=(7, 4))
    sns.heatmap(pivot_delta, annot=True, fmt=".4f", cmap="RdYlGn",
                ax=ax, linewidths=0.5, center=0, vmin=-0.1, vmax=0.1)
    ax.set_title("Delta F1 (Unseen - Baseline) — Domain Adaptation")
    plt.tight_layout()
    plt.savefig(da2_dir / "delta_heatmap.png", dpi=150, bbox_inches="tight")
    plt.show()
...
# ── Chạy Step 12 ──
da2_results = domain_adaptation_unseen(
    MODEL_CONFIGS,
    train_df,
    val_df,
    test_df,
